...
public class RServeExample1 {
     ...
     private static void read_file(String srcPath) throws RserveException, REXPMismatchException {
        ...
        if(extension.equals("log")) {
           	RList x=c.eval("read.table(\""+srcPath+"\", sep=\",\",stringsAsFactors = F, header = F)").asList();

//           	int cols = x.size();
//            int rows = x.at(0).length();
//
//            String[][] s = new String[cols][];
//
//            for (int i = 0; i < cols; i++) {
//                    s[i] = x.at(i).asStrings();
//            }
//
//            for (int i = 0; i < cols; i++) {
//                for (int j = 0; j < rows; j++) {
...
//        }
//           	
////           	System.out.print(c.voidEval("print(data)"));
         } else{
          	c.voidEval("data<-read.csv(src, stringsAsFactors = F, header = F)");
         }
    }   
